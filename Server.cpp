...
void Server::recieveMessageFromClient(const int& fd) {
	...
	if (bytesRecieved <= 0) {
		...
		if (client.hasCompleteMessage()) {
			std::vector<Message> messages = messageHandler.handleMessage(fd, client.getReadBuffer());
			for (size_t i = 0; i < messages.size(); i++)
				resource.findCommand(messages[i].getFirstParam())->execute(resource, messages[i]);
		}
	}
}
